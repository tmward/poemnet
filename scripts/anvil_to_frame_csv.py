...
def get_annotations(directory):
    ...
    return (os.path.join(directory, f)
            for f in os.listdir(directory) if f.endswith('.anvil'))
...
def main():
    ...
    with open(csv_filename, 'w') as csv_file:
        ...
        for video, anno_list in vid_infos.items():
            ...
            # annotator
            sorted_columns = (vid_info.frame_column for vid_info in
                              sorted(anno_list, key=attrgetter('annotator')))
            ...
            for num, frame_row in enumerate(zip_longest(*sorted_columns, fillvalue='NA')):
                csv_file.write(video + ',' + str(num) + ',' + ','.join(frame_row) + '\n')

	...
	if (opts$all) {
		list_tsvs(opts$t) %>%
			...
			walk2(seq_along(.),
						...
						~ save_fingerprint(.x, str_c("video_", str_pad(.y, 2, side=c("left"), pad = "0"), "_fingerprint.", opts$E),
		...
		plot_combined_fingerprint(combo_results[[1]], opts$TITLE1,
															combo_results[[2]], opts$TITLE2) %>%
			save_fingerprint(make_combined_filename(opts$E, opts$VID1, opts$VID2),
											 opts$o, as.numeric(opts$W), as.numeric(opts$H))
	}
}

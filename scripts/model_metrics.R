...
calculate_metrics <- function(df){
	...
	perclass_metrics <- metrics$byClass %>%
		...
		bind_rows(summarise(., Phase = "Overall (Unweighted)", Precision = mean(Precision),
												   Recall = mean(Recall), F1 = mean(F1)),
							summarise(., Phase = "Overall (Weighted)", Precision = sum(Precision*Prevalence),
...
main <- function(tsv_dir, factor_file, results_dir, wants_conf, conf_width, conf_height, conf_ext) {
	...
	if (wants_conf) {
		...
		vid_stats %>%
			...
			map2(seq_along(.),
					 ~ save_confusion_plot(.x, str_c("video_", str_pad(.y, 2, side=c("left"), pad = "0"), "_recall.", conf_ext),
		...
		vid_stats %>%
			...
			map2(seq_along(.),
					 ~ save_confusion_plot(.x, str_c("video_", str_pad(.y, 2, side=c("left"), pad = "0"), "_precision.", conf_ext),
					 											results_dir, conf_width, conf_height))
